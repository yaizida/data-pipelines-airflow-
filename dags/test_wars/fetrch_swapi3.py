...
def fetchswapi():
    ...
    def _fetch_swapi():
        try:
            ...
            # Вставка всех записей в базу данных (с обработкой дубликатов)
            with Session() as session:
                for record in star_wars_records:
                    try:
                        session.add(record)
                        session.commit()
                    except IntegrityError:
                        # Дубликат найден, пропускаем запись
                        session.rollback()
                        print(f"Дубликат найден для персонажа: {record.name}")
                    else:
                        # Запись успешно добавлена
                        print(f"Персонаж добавлен: {record.name}")
